...
#        self.sed_teacher = sed_teacher #deepcopy(sed_student)
        self.opt = opt
    ...
    def training_step(self, batch, batch_indx):
        ...
        if self.wav2vec:
            strong_preds_student, weak_preds_student = self.sed_student(
                features #self.scaler(features) # in Perceptron server, used only features
            )
        else:
            strong_preds_student, weak_preds_student = self.sed_student(
...
#        with torch.no_grad():
#            if self.wav2vec:
#                ema_features = features #self.scaler(features)
#            else:
...
#        self.log("train/teacher/loss_weak", loss_weak_teacher)
    ...
    def validation_step(self, batch, batch_indx):
        ...
        if self.wav2vec:
            logmels = audio #self.scaler(audio)
        else:
...
#        self.log("val/synth/teacher/event_f1_macro", synth_teacher_event_macro)
        ...
        # write log - validation score
        with open("validation_result.txt", 'a+') as f:
            f.write("="*30)
            f.write("\nEpoch: {}".format(self.epoch_check))
            f.write("\nval/weak/student/macro_F1:{}".format(weak_student_f1_macro))
#            f.write("\nval/weak/teacher/macro_F1:{}".format(weak_teacher_f1_macro))
            f.write("\nval/synth/student/intersection_macro_f1:{}".format(intersection_f1_macro_student))
#            f.write("\nval/synth/teacher/intersection_macro_f1:{}".format(intersection_f1_macro_teacher))
            f.write("\nval/synth/student/event_macro_f1:{}".format(synth_student_event_macro))
#            f.write("\nval/synth/teacher/event_macro_f1:{}".format(synth_teacher_event_macro))
